...
| **첫 푸시 (원본 백업, 강제 푸시)**                  | ✅ 완료 (`77aa881`)                                |
| Phase 2: 리팩토링 (아이덴티티 중앙화 + 빌드 안정화) | ✅ 완료 (§9 참조)                                  |
| Phase 1: 콘텐츠 교체                                | ⬜ 예정 (재웅님 이력 데이터 입력 대기)             |
| Phase 3: 배포 파이프라인                            | ⬜ 예정 (Vercel 연결 + jaeung.me DNS)              |
...
## 9. Phase 2 실행 결과 (2026-07-17 완료)

> **원칙: 동작 보존 리팩토링.** 표시 콘텐츠(Jiwon 데이터)는 그대로 두고 아이덴티티를 중앙화. 단, GA/Giscus는 기존에 Jiwon의 속성/저장소를 가리켜서 **기본 비활성**으로 전환(env 설정 시 활성화).

### 아이덴티티 중앙화 (R1/R2/R3/R6)

- **신규 `site.config.ts`** — env(`NEXT_PUBLIC_*`) 기반 배포/런타임 설정. edge OG 라우트·클라이언트 Giscus·GA 등 `data/meta.yml`(fs 런타임)을 못 읽는 곳에서 사용. `.env.example` 제공.
- **신규 `components/Copyright.tsx`** — `Copyright © {올해} {meta.name}` 동적 렌더. `app/layout.tsx`의 하드코딩 2곳 제거.
- **`data/meta.yml`** += `authorHighlight` (Publications 저자 강조를 하드코딩 `"Jiwon Choi"` → 데이터 기반으로).
- 하드코딩 제거처: `app/layout.tsx`(GA·카피라이트), `app/[[...mdxPath]]/page.tsx`(기본 title + `metadataBase`), `app/api/og/route.tsx`(이름·태그라인), `components/Giscus.tsx`(repo/id → env + 조건부 렌더), `components/items/PublicationItem.tsx`(저자 강조).
- **결과: `.tsx/.ts` 코드에 남은 개인 식별자 0건.** 리브랜딩 시 `data/meta.yml` + 환경변수(또는 `site.config.ts` 기본값)만 수정하면 됨.
- 클린업: `ContactButtons` 오타(`valye`→`value`), 주석 `<Search/>` 제거.

### 빌드/런타임 안정화 (R7) — ⚠️ 발견·수정한 기존 레포 버그

검증(`pnpm install/datagen/lint/build`, **Node 20**) 중 원본 레포에 잠재해 있던 오류 3건을 발견·수정:

1. **`generate-data.js` datagen 실패** — `chokidar@5`가 ESM 전용이 되어 `require()` 불가(`ERR_REQUIRE_ESM`). `build`가 datagen을 선행하므로 **빌드 자체가 실패하던 상태**. → watch 모드에서만 쓰는 chokidar를 **동적 import**로 변경.
2. **`pnpm lint` 실패** — Next 16이 `next lint`를 제거. → 스크립트를 `eslint .`로 교체.
3. **ESLint 크래시** — `eslint@10`이 `eslint-config-next@16`(react 플러그인)과 비호환. → 공식 지원 버전 **`eslint@9`로 다운그레이드**. Navigation 스크롤스파이에 타겟 disable, 빌드스크립트/생성물 lint 제외.

- Node 고정: `package.json engines.node ">=20"`, `.nvmrc`(20). `package.json name` → `jaeung-me`. `README.md` 재작성. `metadataBase` 설정으로 OG 절대경로 경고 해소.
- `.gitignore`: `!.env.example` 예외, `.claude/` 세션파일 제외.

### 검증 결과 (Node 20 / pnpm 10.15.1)

| 단계           | 결과                                                                      |
| -------------- | ------------------------------------------------------------------------- |
| `pnpm install` | ✅ 888 packages                                                           |
| `pnpm datagen` | ✅ (chokidar 수정 후) `authorHighlight` 타입 반영 확인                    |
| `pnpm build`   | ✅ exit 0 — 22 라우트 컴파일 + TypeScript 통과 + Pagefind 20페이지 인덱싱 |
| `pnpm lint`    | ✅ exit 0 (eslint 9)                                                      |

### 남은 작업

- **Phase 1 콘텐츠 교체** (재웅님 이력 데이터 입력 후): `data/*.yml`, `content/index.mdx`, `content/projects/**`, `content/blog/posts/**`, `public/` 에셋. + `site.config.ts`/env를 Jaeung 값으로.
- 미착수 선택 항목: R4(섹션 데이터 주도화), R5(ko 정리), R8(profilepic 938KB 최적화).
- **Phase 3 배포**: Vercel 연결 + jaeung.me DNS (§6).
